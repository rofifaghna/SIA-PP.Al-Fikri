<?php 
    if (isset($_POST['simpan'])){
            if ($_POST['status']=='Update'){
              mysql_query("UPDATE rb_nilai_keterampilan SET kd='$_POST[a]', nilai1='$_POST[b]', nilai3='$_POST[d]', nilai5='$_POST[f]', deskripsi='$_POST[h]' where id_nilai_keterampilan='$_POST[id]'");
            }else{
              mysql_query("INSERT INTO rb_nilai_keterampilan VALUES('','$_GET[jdwl]','$_POST[nisn]','$_POST[a]','$_POST[b]','$_POST[d]','$_POST[f]','$_POST[h]','$_SESSION[id]','".date('Y-m-d H:i:s')."')");
            }
        ...
        <div class='box-body'>
          ...
          <div class='panel-body'>
              <table class='table table-bordered table-striped'>
                                ...
                                <tr>
                                  <th style='border:1px solid #e3e3e3; width:110px' ><center>Praktek</center></th>
                                  <th style='border:1px solid #e3e3e3; width:110px' ><center>Proyek</center></th>
                                  <th style='border:1px solid #e3e3e3; width:110px' ><center>Portofolio</center></th>
                                </tr>
                              <tbody>";
                              $no = 1;
                              ...
                              while($r=mysql_fetch_array($tampil)){
                                  ...
                                  if ($_GET['nisn']==$r['nisn']){
                                    ...
                                    $ratarata = average(array($n['nilai1'],$n['nilai3'],$n['nilai5']));
                                    ...
                                    $maxn = mysql_fetch_array(mysql_query("SELECT sum((nilai1+nilai3+nilai5)/3) as rata_rata, deskripsi FROM rb_nilai_keterampilan where kodejdwl='$_GET[jdwl]' AND nisn='$r[nisn]' ORDER BY rata_rata DESC LIMIT 1"));
                                      $cekpredikat1 = mysql_num_rows(mysql_query("SELECT * FROM rb_predikat where kode_kelas='$_GET[id]'"));
                                      if ($cekpredikat1 >= 1){
                                        $grade2 = mysql_fetch_array(mysql_query("SELECT * FROM `rb_predikat` where (".number_format($maxn['rata_rata'])." >=nilai_a) AND (".number_format($maxn['tertinggi'])." <= nilai_b) AND kode_kelas='$_GET[id]'"));
                                      }else{
                                        $grade2 = mysql_fetch_array(mysql_query("SELECT * FROM `rb_predikat` where (".number_format($maxn['rata_rata'])." >=nilai_a) AND (".number_format($maxn['tertinggi'])." <= nilai_b) AND kode_kelas='0'"));
                                      }

                                      
                                      $rapn = mysql_fetch_array(mysql_query("SELECT sum((nilai1+nilai3+nilai5)/3)/count(nisn) as raport FROM rb_nilai_keterampilan where kodejdwl='$_GET[jdwl]' AND nisn='$r[nisn]'"));
